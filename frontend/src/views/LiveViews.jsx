...
export function AnalyticsView() {
  ...
  const tierDistribution = liveData ? [
    { tier: 'Critical', count: liveData.tierCounts.critical, color: '#ef4444' },
    { tier: 'High', count: liveData.tierCounts.high, color: '#f97316' },
    { tier: 'Medium', count: liveData.tierCounts.medium, color: '#eab308' },
    { tier: 'Low', count: liveData.tierCounts.low, color: '#22c55e' },
  ] : [
  ...
  const sourceDistribution = liveData && liveData.sourceDistribution.length ? liveData.sourceDistribution : [
...
export function SensorsView() {
  ...
  const sensors = liveData ? [
    { name: 'Fire Detection - Lobby', status: liveData.simulator ? 'active' : 'inactive', lastPing: '2s ago', type: 'Smoke', zone: 'Main Lobby', signal: 95, uptime: 99.9, icon: '🔥' },
    { name: 'Seismic Monitor', status: 'active', lastPing: '1s ago', type: 'Seismic', zone: 'Foundation', signal: liveData.seismic.magnitude > 0 ? 99 : 50, uptime: 100, icon: '📊' },
    { name: 'Weather Station', status: 'active', lastPing: '0s ago', type: 'Weather', zone: 'Roof', signal: 98, uptime: 100, icon: '🌤️' },
    { name: 'Water Level - Basement', status: liveData.simulator ? 'active' : 'inactive', lastPing: '5s ago', type: 'Flood', zone: 'Basement B1', signal: 87, uptime: 98.2, icon: '💧' },
...
export function AlertsView() {
  ...
  const alerts = liveData ? [...liveData.map((a, i) => ({ ...a, id: 'live-'+i })), ...baseAlerts] : baseAlerts;
...
export function SettingsView() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/settings`)
      .then(res => res.json())
      .then(data => setAutoFeed(data.autoFeedEnabled))
      .catch(err => console.error(err));
  }, []);

  const handleToggleAutoFeed = () => {
    const newValue = !autoFeed;
    setAutoFeed(newValue);
    fetch(`${API_BASE_URL}/api/settings`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ autoFeed: newValue })
    });
  };
